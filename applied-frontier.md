...
And here is what DeepSeek said about serving it on H800 nodes (8 GPUs, 80GB each, 200GB/s of NVLink egress per GPU, which is the 400GB/s bidirectional figure NVIDIA quotes and of which DeepSeek measures about 160, and one 400Gb/s or 50GB/s InfiniBand NIC per GPU) in the 24 hours ending noon on February 28, 2025:
...
Put expert parallelism inside the rack. Take 64-way EP (six experts per GPU; the other 8 GPUs of each rack can carry redundant copies of hot experts, as DeepSeek's serving system and Kimi K3's training system do, or you can run 72-way with five or six experts per GPU; the arithmetic below barely changes) and FSDP across the 144 EP groups. The FSDP threshold becomes `(384 / (6 * 64)) * 1390 = 1,390` tokens per GPU with bf16 gathers, or 694 with fp8 gathers, against 10,200: **compute-bound with a 7x margin**. The expert AllToAll inside the rack needs $F > \alpha_\text{fp8} / 2 = 5560 / 2 = 2{,}780$ with fp8 matmuls and fp8 dispatch, and V4-Pro's experts are 3,072 wide, so it is **compute-bound by 10%** without any overlap at all. DeepSeek's own condition for hiding the AllToAll behind expert compute, $C / W \leq 2F = 6{,}144$ FLOPs per byte, is met by the NVL72's 5,560. On paper, then, nothing in this plan is communication-bound, and the ICI overlap gymnastics of [Section 13](../moe) aren't needed. What sets the utilization instead is the pipeline (we haven't needed pipeline parallelism for memory, but every lab uses it to keep the data-parallel axis small and to interleave micro-batches), the attention layers, the HBM traffic of the sparse-attention indexer, and how close the grouped expert matmuls get to peak with 10,200 tokens spread over six experts per GPU. Note the two cliffs you must not step off: an EP group that spills into a second rack drops from 900GB/s to 50GB/s per GPU, an 18x loss, and a cluster of 8-GPU HGX B200 nodes instead of racks would put the AllToAll on InfiniBand at $\alpha_\text{fp8} = 90{,}000$, where nothing is compute-bound.
...
<p markdown=1 class="takeaway">**Takeaway:** A 2026 frontier MoE is about a week of pretraining on 128 GB200 NVL72 racks at 40% utilization. The plan is expert parallelism of about $E/k$ inside each rack, where NVLink keeps the AllToAll compute-bound for 3,072-wide experts even in fp8, and FSDP or data parallelism across racks, where the rack's 3.6TB/s of InfiniBand egress makes the weight gathers cheap. Memory is irrelevant at this scale, and the rooflines say nothing is communication-bound; the same model on 2,048 H800s at DeepSeek-V3's efficiency would take five months.</p>

## The Same Run on a TPU7x Pod

Suppose instead we had a full TPU7x pod: 9,216 chips, 192GB and 7.4e12 bytes/s of HBM each, 4.61e15 fp8 FLOPs/s, a 3D torus with 1.8e11 bytes/s per axis, so $\alpha = C / W_\text{ici}$ is `2.3e15 / 1.8e11 = 12,800` per axis in bf16 and 25,600 in fp8. The pod delivers `9216 * 4.61e15 = 4.2e19` FLOPs/s at peak, so at 40% the run takes `9.7e24 / (4.2e19 * 0.4) = 5.7e5 s`, **6.6 days**, within 10% of the GB200 cluster. The difference is in what the network allows.
...
10% of 9.7e24 is 9.7e23 FLOPs. Each generated token costs `2 * 49e9` FLOPs to generate and `6 * 49e9` to train on, `8 * 49e9 = 3.9e11` in all, so the budget buys **about 2.5 trillion generated tokens**, about 8% of the pretraining corpus, produced one token at a time. At 10,000 tokens per second per accelerator (a generous figure for a 49B-active model at long context) the 9,216-way machine generates 9.2e7 tokens per second and needs `2.5e12 / 9.2e7 = 27,000 s`, **about 7.5 hours** if every GPU or chip decoded at full rate for the whole time. The FLOPs aren't the problem. The problem, per [Section 15](../training-2026), is that generation runs at a fraction of the pod's efficiency and can't finish before its longest response does, so the calendar time is several times the FLOPs time.
...
**Question 5 [the fp8 gather question]:** In the GB200 training plan above, the FSDP threshold across racks was 1,390 tokens per GPU if weights are gathered in bf16 and 694 if gathered in fp8; in the TPU7x plan it was 8,500 and 4,300 per chip. DeepSeek's recipe keeps fp32 master weights and quantizes to fp8 per 128x128 block before each matmul. Design where in the FSDP gather the quantization should happen so that ICI carries fp8, and say what extra memory or compute it costs.
